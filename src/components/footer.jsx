...
export default function Footer() {
    ...
    const LINKS = [
        ...
        {
            title: "Company",
            items: ["Contact", "About us", "Press", "News"],
        },
        {
            title: "Resource",
            items: ["Blog", "Newsletter", "Events", "Help"],
        },
    ];
    const currentYear = new Date().getFullYear();
    return (
        <footer className={`w-full bg-white p-2 md:p-8 md:px-24 my-10  ${!isShow ? 'hidden' : ''}`}>
                    ...
                    <div className="grid grid-cols-3 justify-between gap-4">
                        {LINKS.map(({ title, items }) => (
                            <ul key={title}>
                                <Typography
                                    variant="small"
                                    className="mb-3 text-[var(--primary-color)] font-medium"
                                >
                                    {title}
                                </Typography>
                                {items.map((link) => (
